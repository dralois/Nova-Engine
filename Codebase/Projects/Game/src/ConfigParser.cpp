	...
	if (l_strSrc.is_open())
	{
		...
		while (!l_strSrc.eof()) {			
			...
			else if (name == "Mesh")
			{
				...
				l_strSrc >> meshDictionary[name].File >> meshDictionary[name].Diffuse >> meshDictionary[name].Specular >> 
					meshDictionary[name].Glow >> meshDictionary[name].Normal >> meshDictionary[name].Transparency;
			}
